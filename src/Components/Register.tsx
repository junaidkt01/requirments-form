...
//     try {
//       const userCred = await createUserWithEmailAndPassword(auth, email, pw);
//       await saveUserProfile(userCred.user.uid, email, phone || undefined);
//       // send email verification
//       await sendEmailVerification(userCred.user);
//       setSentVerification(true);
//     } catch (err: any) {
...
//         <div className="field">
//           <label className="label">Your Email</label>
//           <input className={`input ${email && !isValidEmail(email) ? "invalid" : ""}`}
//                  value={email} onChange={e=>setEmail(e.target.value)} />
//         </div>

//         <div className="field">
//           <label className="label">Password</label>
//           <input type="password" className={`input ${pw && !isStrongPassword(pw) ? "invalid" : ""}`}
...
//         <div className="field">
//           <label className="label">Confirm Password</label>
//           <input type="password" className={`input ${pw2 && pw!==pw2 ? "invalid" : ""}`} value={pw2} onChange={e=>setPw2(e.target.value)} />
//         </div>

//         <div className="field">
//           <label className="label">Phone (optional)</label>
//           <input value={phone} onChange={e=>setPhone(e.target.value)} className={`input ${phone && !isValidPhone(phone) ? "invalid" : ""}`} placeholder="e.g. 971501234567" />
//           <div className="helper">We store this for your profile and future phone OTP (if enabled)</div>
//         </div>

//         {error && <div className="error">{error}</div>}
//         {sentVerification && <div className="helper">Verification sent — check your email and click the verification link.</div>}

//         <div className="actions">
//           <button className="btn" type="submit" disabled={loading}>{loading ? "Creating..." : "Continue"}</button>
//         </div>
//       </form>
//     </div>
//   );
